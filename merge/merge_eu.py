#!/usr/bin/env python3
"""
Merge all *non-Greek* CSVs from ../data by ('geo','year') and produce
STACKED outputs + a full data-quality report.

Inclusion rule: every *.csv in ../data except those with stem starting "greek_".
(So it'll pick up things like EU_labor_market_*.csv, precarious_*.csv, etc.)

Outputs:
  1) merged_by_geo_eu_stacked.csv         -> STACKED view (keeps distinct rows) + 'source'
  2) _dq_eu/DATA_QUALITY_REPORT.md        -> Human-readable DQ report (pre-merge + stacked summary)
  3) _dq_eu/*.csv                         -> Supporting diagnostics (missingness, etc.)
  4) _dq_eu/geo_labels_L{0,1,2,3}.csv     -> For each NUTS level: every geo with *all* its label variants
  5) _dq_eu/geo_labels_all_levels.csv     -> Combined view (all levels)
"""
...
# -------- Config --------
EXCLUDE_PREFIXES = ("greek_",)      # exclude Greek tourism files
INCLUDE_SUFFIX = ".csv"             # include only CSVs
...
OUT_STACKED = OUT_DIR / "merged_data/merged_by_geo_eu_stacked.csv"
LOG_FILE = OUT_DIR / "logs/merge_log_eu.txt"

# Data-quality outputs directory (separate from Greek one)
DQ_DIR = OUT_DIR / "_dq_eu"
...
def normalize_geo(val):
    """
    Accept NUTS-like codes or names; return a canonical key usable across files.
    - NUTS-ish (e.g., EL30, GR112, DE21): uppercase, no spaces.
    - Otherwise: accent-stripped, single-spaced, casefolded (for name matches).
    """
    if pd.isna(val):
        return pd.NA
    s = str(val).strip()
    s = unicodedata.normalize("NFKC", s)
    nutsish = re.fullmatch(r"[A-Za-z]{2}\s*[\dA-Za-z]+", s)  # e.g., EL30, GR112, DE21
    if nutsish:
...
def main():
    ...
    # Collect *non-Greek* CSVs
    all_csvs = sorted(p for p in DATA_DIR.glob(f"*{INCLUDE_SUFFIX}") if not p.stem.startswith(EXCLUDE_PREFIXES))
    if not all_csvs:
        raise SystemExit(f"No eligible CSV files found in {DATA_DIR} (excluding prefixes {EXCLUDE_PREFIXES}).")
    ...
    for fp in all_csvs:
        try:
            # Robust read with encoding sniff
            last_err = None
            used_enc = None
            ...
            # NEW: lock dtypes before any string/regex filters or joins
    ...
    if not dfs_stacked:
        raise SystemExit("No dataframes to stack (all files failed). See merge_log_eu.txt for details.)")

    # -------- Build and write STACKED output --------
    stacked = pd.concat(dfs_stacked, ignore_index=True, sort=False)
    lead_stacked = ["geo", "year", "nuts_level", "nuts_label", "source"]
    other = [c for c in stacked.columns if c not in lead_stacked]
    stacked = stacked[lead_stacked + other].sort_values(["geo", "year", "source"]).reset_index(drop=True)
    ...
    #    (keeps the first occurrence even if the duplicate came from another file)
    ...
    # quick exports (like Greek script names, but under _dq_eu/)
    nuts_levels_df = (
    ...
    lines = []
    lines.append(f"# Data Quality Report (Non-Greek Merge — STACKED only)")
    ...
    for stem, rpt in pre_reports.items():
        ...
        lines.append(f"- Missingness CSV: `_dq_eu/{stem}__missingness.csv`")
        lines.append("")
    ...
    lines.append(f"- Full missingness CSV: `_dq_eu/stacked__missingness.csv`")
